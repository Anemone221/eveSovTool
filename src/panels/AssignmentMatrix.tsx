...
export function AssignmentMatrix() {
    ...
    const [alnSuggestions, setAlnSuggestions] = useState<
    ...
    useEffect(() => {
        void Promise.all(
            FMT_KEYS.map((k) => evesov.prefs.get(FMT_PREF_PREFIX + k)),
        ).then((vals) => {
            setFmt((prev) => {
                const next = { ...prev };
    ...
    const onCellClick = useCallback(
        async (
            ...
            if (!has) {
                ...
                if (!r.ok) {
                    alert(r.error ?? "Failed to assign upgrade.");
                    return;
                }
                if (upgradeName === "Advanced Logistics Network") {
                    const { targets } = await evesov.plans.getAlnTargets(
                        activePlanId,
                        systemId,
                    );
                ...
                await evesov.plans.setUpgradeInstalled(
                    activePlanId,
                    systemId,
                    upgradeName,
                    true,
                ...
                await evesov.plans.removeUpgrade(
                    activePlanId,
                    systemId,
                    upgradeName,
                );
    ...
    if (activePlanId === null) {
        return (
            <div className="overview overview--empty">
                Activate a plan to see its assignment matrix.
            </div>
        );
    }
    if (!matrix)
        return <div className="overview overview--empty">Loading…</div>;
    if (matrix.systems.length === 0) {
        return (
            <div className="overview overview--empty">
                No systems in plan yet. Add scopes from the Universe tree or
                assign upgrades from System detail.
            </div>
        );
    }
                ...
                <table className={tableClass}>
                    <thead>
                        <tr>
                            ...
                            {showUsageCol && (
                                <th className="matrix__sticky-col matrix__usage-header">
                                    Usage
                                </th>
                            )}
                            <th
                        ...
                        <tr className="matrix__totals-row">
                            <th className="matrix__sticky-col matrix__totals-label">
                                Totals
                            </th>
                            {showUsageCol && (
                        ...
                        {alnPending && (
                            <tr className="matrix__aln-drawer">
                                ...
                                {showUsageCol && (
                                    <td className="matrix__usage-cell" />
                                )}
                                <td
                                    colSpan={columns.length}
                                    className="matrix__aln-drawer__body"
                                >
                    ...
                    <tbody>
                        {matrix.systems.map((s) => {
                            const installedMap = new Map(
                                s.upgrades.map((u) => [u.name, u.installed]),
                            );
                            return (
                                <tr key={s.id}>
                                    <td className="matrix__sticky-col matrix__system-cell">
                                        <div className="matrix__system-name-row">
                                            ...
                                            {s.status !== "local" && (
                                                <span
                                                    className={`status-tag status-tag--${s.status}`}
                                                    title={`Workforce: ${s.status}`}
                                                >
                                                    {s.status}
                                                </span>
                                            )}
                                    ...
                                    {showUsageCol && (
                                        <td className="matrix__usage-cell">
                                            {!opsec.powerHideVisual && (
                                                <MiniMeter
                                                    label="P"
                                                    consumed={s.consumedPower}
                                                    available={s.availablePower}
                                                />
                                            )}
                                            {!opsec.workforceHideVisual && (
                                                <MiniMeter
                                                    label="W"
                                                    consumed={
                                                        s.consumedWorkforce
                                    ...
                                    {columns.map((c) => {
                                        ...
                                        if (!has) {
                                            ...
                                            c === "Advanced Logistics Network"
                                        ) {
                                            const link = s.alnLink;
                                            const linkLabel = link
                                                ? `→ ${link.linkedSystemName}`
                                                : "link required";
                                            title = installed
