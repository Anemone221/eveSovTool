...
import type { PlanStructure, StructureAddPayload, StructureNode, StructureType, StructureLocation } from '@shared/index';
...
function inferType(line: string): { structureType: StructureType; location: StructureLocation | null } {
  const l = line.toLowerCase();
  if (l.includes('ansiblex')) return { structureType: 'Ansiblex', location: 'Gate' };
  if (l.includes('metenox')) return { structureType: 'Metenox', location: 'Moon' };
  if (l.includes('athanor')) return { structureType: 'Athanor', location: 'Moon' };
  if (l.includes('tatara')) return { structureType: 'Tatara', location: 'Moon' };
  if (l.includes('sotiyo')) return { structureType: 'Sotiyo', location: 'Deep' };
...
export function registerStructuresIpc(): void {
  ipcMain.handle(
    ...
    (_, planId: number, systemId?: number): StructureNode[] => {
      const db = getDb();
      const rows = systemId != null
        ? db.prepare(LIST_BY_SYSTEM_SQL).all({ planId, systemId }) as StructureDbRow[]
        : db.prepare(LIST_SQL).all({ planId }) as StructureDbRow[];
  ...
  ipcMain.handle(
    'structures.add',
    (_, planId: number, systemId: number, structure: StructureAddPayload): { id: number } => {
      const db = getDb();
      const result = db.prepare(
        `INSERT INTO plan_structures (plan_id, system_id, structure_type, name, location, notes, source)
         VALUES (?, ?, ?, ?, ?, ?, 'manual')`
      ).run(planId, systemId, structure.structureType, structure.name ?? null, structure.location ?? null, structure.notes ?? null);
  ...
  ipcMain.handle(
    ...
    (_, planId: number, structureId: number): void => {
      const db = getDb();
      db.prepare('DELETE FROM plan_structures WHERE id = ? AND plan_id = ?').run(structureId, planId);
      broadcastPlanChanged(planId);
    }
  );
  ...
  ipcMain.handle(
    ...
    (_, planId: number, systemId: number, text: string): { count: number } => {
      const db = getDb();
