...
import type { StructureNode, StructureType, StructureLocation } from '@shared/index';
...
export function StructuresPage() {
  ...
  const refresh = useCallback(async () => {
    if (activePlanId == null) { setNodes([]); return; }
  ...
  const commitImport = async () => {
    ...
    const { count } = await evesov.structures.importClipboard(activePlanId, importing.systemId, importing.text);
    setImporting({ ...importing, result: `Imported ${count} structure${count !== 1 ? 's' : ''}.` });
  };
  ...
  if (activePlanId == null) {
    return <div className="structures"><p className="structures__empty">Select a plan to view structures.</p></div>;
  }

  // Group nodes by constellation
  const byConstellation = new Map<number, { constellationName: string; regionName: string; nodes: StructureNode[] }>();
  ...
  return (
    <div className="structures">
      {byConstellation.size === 0 && (
        <p className="structures__empty">No structures planned. Use the add button next to a system to get started.</p>
      )}
      {Array.from(byConstellation.entries()).map(([cid, group]) => {
        const totalCount = group.nodes.reduce((s, n) => s + n.structures.length, 0);
        ...
        return (
          <div key={cid} className="structures__constellation">
            ...
            >
              <span className="structures__chevron">{isCollapsed ? '▸' : '▾'}</span>
              <span className="structures__constellation-name">{group.constellationName}</span>
              <span className="structures__region-name">({group.regionName})</span>
              <span className="structures__count">{totalCount}</span>
            </button>
            {!isCollapsed && (
              <div className="structures__systems">
                {group.nodes.map((node) => (
                  <div key={node.systemId} className="structures__system">
                    <div className="structures__system-header">
                      <span className="structures__system-name">{node.systemName}</span>
                      <button
                        type="button"
                        className="structures__btn-add"
                        title="Add structure"
                        onClick={() => startAdd(node.systemId)}
                      >
                        + Add
                      </button>
                      <button
                        ...
                        onClick={() => startImport(node.systemId)}
                      >
                        ⎘ Import
                      </button>
                    </div>
                    {adding?.systemId === node.systemId && (
                      <form
                        ...
                        onSubmit={(e) => { e.preventDefault(); void commitAdd(); }}
                      >
                        <select
                          value={adding.type}
                          onChange={(e) => setAdding({ ...adding, type: e.target.value as StructureType })}
                        >
                        ...
                        <select
                          ...
                          onChange={(e) => setAdding({ ...adding, location: e.target.value as StructureLocation | '' })}
                        >
                    ...
                    {importing?.systemId === node.systemId && (
                      <div className="structures__import-form">
                        <textarea
                          rows={4}
                          placeholder="Paste structure names, one per line"
                          value={importing.text}
                          onChange={(e) => setImporting({ ...importing, text: e.target.value, result: null })}
                        />
                        <div className="structures__import-actions">
                          <button type="button" onClick={() => void commitImport()}>Import</button>
                          <button type="button" onClick={() => setImporting(null)}>Cancel</button>
                          {importing.result && <span className="structures__import-result">{importing.result}</span>}
                        </div>
                      </div>
                    )}
                      ...
                      {node.structures.map((s) => (
                        <li key={s.id} className="structures__card">
                          ...
                          <button
                            ...
                            onClick={() => void removeStructure(s.id)}
                          >
                            ×
                          </button>
                        </li>
      ...
      {nodes.length === 0 && activePlanId != null && (
        <SystemlessAdder activePlanId={activePlanId} onRefresh={refresh} />
      )}
    </div>
  );
}

function SystemlessAdder({ activePlanId, onRefresh }: { activePlanId: number; onRefresh: () => Promise<void> }) {
  const [query, setQuery] = useState('');
  const [results, setResults] = useState<{ systemId: number; systemName: string }[]>([]);
  const [adding, setAdding] = useState<AddFormState | null>(null);
  const [importing, setImporting] = useState<ImportFormState | null>(null);
    ...
    <div className="structures__search-add">
      ...
      {results.map((r) => (
        <div key={r.systemId} className="structures__search-result">
          ...
          <button type="button" onClick={() => startAdd(r.systemId)}>+ Add</button>
          <button type="button" onClick={() => startImport(r.systemId)}>⎘ Import</button>
